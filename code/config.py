...
def parse_args():
    ...
    parser.add_argument('--optimized_letter', type=str, default="[0]", help="the list of letters in the word to optimize")
    ...
    cfg.optimized_letter = list(map(int, args.optimized_letter.strip('[]').split(',')))
    cfg.letter_name = ''.join([str(elem) for elem in cfg.optimized_letter])
    ...
    cfg.letter = f"{cfg.font}_{cfg.word}_{cfg.letter_name}_scaled"
