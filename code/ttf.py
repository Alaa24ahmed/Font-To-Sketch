    ...
    for segment in commands:
        ...
        if segment_type  == "Move":
            ...
            if(segment.length() >= thresh):
                new_curve1, new_curve2 = split_cubic_bezier(segment.start, segment.control1, segment.control2, segment.end)
...
def font_string_to_svgs(dest_path, font, txt, size=30, spacing=1.0, target_control=None, subdivision_thresh=None):
    ...
    font_string_to_beziers(svg_path, txt, target_control=target_control)
...
def combine_word_mod(word, letter, font, experiment_dir):
    ...
    if cfg.use_wandb:
        wandb.log({"Compined": wandb.Image(f"{experiment_dir}/{font}_{word}_{letter}.png")})
