...
for i in "${!words[@]}"; do
    ...
    for mx in $(seq 0 $n); do 
        for mn in $(seq 0 $mx); do 
            ...
            python code/main.py --experiment $experiment --script $script --semantic_concept "${concepts[i]}" --optimized_region "$optimized_region" --word "${word[i]}" --seed $seed --font $font --use_wandb 0 --wandb_user "graduation-word-as-image" --use_dot_product_loss 1
        done
